...
class WxuserApi extends Api{
     ...
     * 连接查询默认收货地址
     */
    const FIND_DEFAULT_ADDRESS = 'Weixin/Wxuser/finddaddre';
...
where wu.status =  1 limit ".$page['curpage'] . ',' . $page['size'];
    ...
    public function finddaddre($map,$field= false){
        $result = $this->model->join('itboye_address on itboye_wxuser.default_address = itboye_address.id')->where($map)->field($field)->find();
        if ($result === false) {
            $error = $this -> model -> getDbError();
            return $this -> apiReturnErr($error);
        } else {
