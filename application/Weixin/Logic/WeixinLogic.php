...
class WeixinLogic{
     ...
     * @internal param token $accessToken
     */
    public function getTcode($scene_id) {
        $accessToken = $this -> getAccessToken();
        $url = "https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=" . $accessToken;
        $data = array("expire_seconds" => '2592000',"action_name" => "QR_SCENE", "action_info" => array('scene' => array('scene_id' => $scene_id)));
        $obj = $this -> curlPost($url, json_encode($data));
     ...
     * 网页登陆获取用户信息
     * */
    public function webGetUserInfo($openid,$accessToken) {
	 ...
	 */
    public function getOAuth2BaseURL($redirectURL, $state = '_openid_', $scope = 'snsapi_userinfo') {
	...
	public function getOAuth2AccessToken($code) {
        ...
        //$url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=wx1fb9ef21333c757f&secret=d344d5b839211c3ca2d6f95c3c485d95&code=$code&grant_type=authorization_code";
        $arr = json_decode($this -> curlGet($url), true);
		return $arr;
	}
	 ...
	 * 走模板消息接口
	 */
	public function sendTmplMsgToFans($data) {
        $AccessToken =  $this ->getAccessToken();

        $url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=".$AccessToken;
        $json = json_encode($data, JSON_UNESCAPED_UNICODE);
        $sendinfo = $this -> curlPostArray($url,$json);
        return $sendinfo;
	}
...
//		$header = "Accept-Charset: utf-8";
     ...
     * 数组形式返回
     * */
    protected function curlPostArray($url, $data) {
